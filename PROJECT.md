...
| 3 | M3: 備份與安全刪除 | 自動備份並安全刪除舊全域技能目錄，確保 agy 載入 25 個最新指標的技能 | M2 | DONE |
| 4 | M4: 設定修正防衝突 | 修正專案或外掛設定，避免再次發生雙重路徑載入衝突 | M3 | DONE |
| 5 | M5: 驗證機制 | 驗證 GitHub Release 與 `agy plugin install` 是否自動 checkout 到 `v1.6.0` 標記或 clone 預設分支，確認 `SKILL.md` 為新版 | M4 | DONE |
